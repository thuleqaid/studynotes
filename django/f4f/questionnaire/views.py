...
def _doQuiz_T3(request,titleid):
    if 'LASTQINDEX' in request.session:
        # 2nd~last question
        if request.POST['submit']=='yes':
            choice=Answer.objects.filter(question_id=int(request.session['LASTQINDEX'])).filter(score__gt=0)
        else:
            choice=Answer.objects.filter(question_id=int(request.session['LASTQINDEX'])).filter(score__lt=1)
        request.session['SCORE']+=choice[0].score
        request.session['SEQ'].append((request.session['LASTQINDEX'],choice[0].aindex))
    else:
        # 1st question
        request.session['SEQ']=[]
        request.session['SCORE']=0
    ...
    if request.session['QQINDEX']>=len(request.session['QQUEUE']) :
        # last question
        resulttype,score=_save_log(titleid,request.session['SCORE'],request.session['SEQ'])
        c=RequestContext(request,{'titletype':'T3','resulttype':resulttype,'score':score})
        return render_to_response('questionnaire/result.html',c)
    else:
        qqindex=request.session['QQINDEX']
        question=Question.objects.get(title_id=titleid,qindex=request.session['QQUEUE'][qqindex])
        request.session['QQINDEX']+=1
        request.session['LASTQINDEX']=question.id
        choices=question.current.filter(score__gt=0)
        c=RequestContext(request,{'question':question,'choices':choices[0]})
        return render_to_response('questionnaire/quiz3.html',c)
